...
export default function DecisionActionTracker({ priorityRows = [] }) {
  ...
  const currentForecastByBarangay = useMemo(() => {
              ...
              {isLoading ? (
                ...
                recentActions.map((action) => {
                  ...
                  const riskHasChanged = Boolean(
                    currentForecastRisk &&
                    action.risk_level &&
                    currentForecastRisk !== action.risk_level
                  )
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className={`rounded-full border px-3 py-1.5 text-xs font-black ${getRiskBadgeStyle(action.risk_level)}`}>
                              Risk at creation: {action.risk_level}
                            </span>
                            {currentForecastRisk && (
                              <span className={`rounded-full border px-3 py-1.5 text-xs font-black ${getRiskBadgeStyle(currentForecastRisk)}`}>
                                Current forecast: {currentForecastRisk}{riskHasChanged ? ' · changed' : ''}
                              </span>
                            )}
