    ...
    <div class="risk-tooltip-card">
      ...
      ${
        layerMode === 'hotspot'
          ...
          : `
            <div class="risk-tooltip-row">
              <span>Combined priority score</span>
  ...
  return `
    <div class="barangay-detail-card">
      ...
      <div class="barangay-decision-banner ${priorityClassName}">
        ...
        <strong>${escapeHtml(decision.priority)}</strong>
        <small>Decision score: ${escapeHtml(formatNumber(decision.score))} points · Combined priority score: ${escapeHtml(formatRiskScore(decision.multiSourceRiskScore))}</small>
      </div>
...
function InvalidateMapSize({
  ...
  useEffect(() => {
    ...
    const invalidate = () => {
      ...
      animationFrame = window.requestAnimationFrame(() => {
        ...
        if (refitBoundary && geoJson?.features?.length) {
          try {
            ...
            console.warn('Unable to refit boundary after map resize:', error)
          }
        }
      })
    }

    // Handle the initial layout transition and delayed side-panel/grid changes.
    const firstTimer = window.setTimeout(invalidate, 80)
    const secondTimer = window.setTimeout(invalidate, 320)

    // The Map page can change column widths when detail panels open without
    // changing the window size. Observe the actual Leaflet container so its
    // internal viewport is kept in sync without re-downloading any map data.
  ...
  }, [layoutKey, mapStyle, geoJson, refitBoundary, map])
    ...
    return {
      color: isSelected
        ...
        : isContextOnly
          ? '#64748b'
          : colors.border,
      weight: isSelected ? 4 : row ? 2.4 : isContextOnly ? 1.6 : 1.5,
      opacity: isSelected ? 1 : isContextOnly ? 0.82 : 0.95,
      fillColor: isContextOnly ? '#475569' : colors.fill,
        ...
        : row
          ...
          : isContextOnly
            ...
            : isSatellite
              ? 0.08
              : 0.14,
      dashArray: isContextOnly ? '' : row ? '' : '5 5',
      lineJoin: 'round',
      lineCap: 'round',
