...
export function getDensityLevel(density) {
  ...
  if (value >= 5000) return densityLevels.VERY_HIGH
  if (value >= 1500) return densityLevels.HIGH
  if (value >= 500) return densityLevels.MODERATE
...
export function getPopulationExposure(population) {
  ...
  if (value <= 0) {
    return { label: 'Population data unavailable', score: 0, componentScore: 0 }
  }

  if (value >= 15000) {
    return { label: 'High population exposure', score: 3, componentScore: 8 }
  }

  if (value >= 8000) {
    return { label: 'Moderate population exposure', score: 2, componentScore: 5 }
  }

  return { label: 'Lower population exposure', score: 1, componentScore: 2 }
}
...
export function getRainfallPressure({ averageRainfall = 0, totalRainfall = 0 } = {}) {
  ...
  // The persisted forecast profile is based on average rainfall. totalRainfall is
  // retained only for backward-compatible callers and is not independently scored.
  const value = averageValue > 0 ? averageValue : totalValue

  if (value <= 0) {
    return { label: 'Rainfall data unavailable', score: 0, componentScore: 0 }
  }

  if (value >= 80) {
    return { label: 'High rainfall pressure', score: 3, componentScore: 10 }
  }

  if (value >= 20) {
    return { label: 'Moderate rainfall pressure', score: 2, componentScore: 7 }
  }

  return { label: 'Low rainfall pressure', score: 1, componentScore: 3 }
}
...
export function getTemperatureSuitability(temperature) {
  ...
  if (value <= 0) {
    return { label: 'Temperature data unavailable', score: 0, componentScore: 0 }
  }

  if (value >= 24 && value <= 32) {
    return { label: 'Temperature suitable for mosquito activity', score: 3, componentScore: 10 }
  }

  if ((value >= 20 && value < 24) || (value > 32 && value <= 35)) {
    return { label: 'Temperature partly suitable for mosquito activity', score: 2, componentScore: 6 }
  }

  return { label: 'Temperature less suitable for mosquito activity', score: 1, componentScore: 2 }
}
...
export function getHumiditySuitability(humidity) {
  ...
  if (value <= 0) {
    return { label: 'Humidity data unavailable', score: 0, componentScore: 0 }
  }

  if (value >= 80) {
    return { label: 'High humidity suitability', score: 3, componentScore: 10 }
  }

  if (value >= 60) {
    return { label: 'Moderate humidity suitability', score: 2, componentScore: 7 }
  }

  return { label: 'Low humidity suitability', score: 1, componentScore: 3 }
}

export function getEnvironmentalSuitability(input = {}) {
  const rainfallPressure = getRainfallPressure({
    averageRainfall:
      input.averageRainfall ?? input.avgRainfall ?? input.rainfall ?? input.rainfallAverage,
    totalRainfall: input.totalRainfall ?? input.rainfallTotal,
  })
  const temperatureSuitability = getTemperatureSuitability(
    input.averageTemperature ?? input.avgTemperature ?? input.temperature ?? input.temperatureAverage
  )
  const humiditySuitability = getHumiditySuitability(
    input.averageHumidity ?? input.avgHumidity ?? input.humidity ?? input.humidityAverage
  )

  const componentScore =
    toNumber(rainfallPressure.componentScore) +
    toNumber(temperatureSuitability.componentScore) +
    toNumber(humiditySuitability.componentScore)
  const availableFactors = [
    rainfallPressure.componentScore,
    temperatureSuitability.componentScore,
    humiditySuitability.componentScore,
  ].filter((value) => toNumber(value) > 0).length
  ...
  if (componentScore >= 24) label = environmentalLevels.HIGH
  else if (componentScore >= 16) label = environmentalLevels.MODERATE

  return {
    label,
    // Keep this compact score for existing decision-support weighting.
    score: label === environmentalLevels.HIGH ? 3 : label === environmentalLevels.MODERATE ? 2 : 1,
    componentScore,
    normalizedScore: Math.round((componentScore / 30) * 100),
    rainfallPressure,
    temperatureSuitability,
    humiditySuitability,
  }
...
export function computeMultiSourceRisk(input = {}) {
  const forecast = toNumber(
    input.forecast ?? input.forecastedCases ?? input.predictedCases ?? input.projectedCases
  )
  const currentCases = toNumber(
    input.currentCases ?? input.current_cases ?? input.latestCases ?? input.latest_cases
  )
  const previousCases = toNumber(
    input.previousCases ?? input.previous_cases ?? input.lastPeriodCases
  )
  const population = toNumber(
    input.population ?? input.totalPopulation ?? input.populationCount
  )
  const areaSqKm = toNumber(
    input.area_sqkm ?? input.areaSqKm ?? input.areaKm2 ?? input.area
  )
  ...
  const riskLevelComponent = risk === riskLevels.HIGH ? 40 : risk === riskLevels.MODERATE ? 25 : 10
  const forecastVolumeComponent = Math.min(15, Math.round((forecast / 8) * 100) / 100)
    ...
    trendDirection === trendDirections.INCREASING
      ...
      : trendDirection === trendDirections.STABLE
        ...
        : trendDirection === trendDirections.DECREASING
          ? 1
          : 0
  const populationComponent = toNumber(populationExposure.componentScore)
  const densityComponent =
    density <= 0 ? 0 : density >= 5000 ? 7 : density >= 1500 ? 5 : density >= 500 ? 3 : 1
  const environmentalComponent = toNumber(environmentalSuitability.componentScore)
  const score = Math.round(
    clamp(
      riskLevelComponent +
        forecastVolumeComponent +
        trendComponent +
  ...
  return {
    ...
    components: {
      ...
      forecastVolume: forecastVolumeComponent,
      trend: trendComponent,
      rainfall: toNumber(environmentalSuitability.rainfallPressure.componentScore),
      temperature: toNumber(environmentalSuitability.temperatureSuitability.componentScore),
      humidity: toNumber(environmentalSuitability.humiditySuitability.componentScore),
      environment: environmentalComponent,
...
function buildRationale({
  ...
  if (toNumber(multiSourceRiskScore) > 0) {
    reasons.push(`Multi-source prioritization score is ${toNumber(multiSourceRiskScore)} out of 100.`)
  }
